...
impl<'m, 's, S: Source> Context<'m, 's, S> {
    ...
    fn func_from_idx(&self, idx: u32, when: &'static str, offset: usize) -> Result<&'m Func<'_>, S> {
        self.validate_idx(&self.module.funcs, idx, "function", when, offset)
    }

    fn table_from_idx(&self, idx: u32, when: &'static str, offset: usize) -> Result<&'m Table<'_>, S> {
        self.validate_idx(&self.module.tables, idx, "table", when, offset)
    }

    fn global_from_idx(&self, idx: u32, when: &'static str, offset: usize) -> Result<&'m Global<'_>, S> {
        self.validate_idx(&self.module.globals, idx, "global variable", when, offset)
    }

    fn memory_from_idx(&self, idx: u32, when: &'static str, offset: usize) -> Result<&'m Memory<'_>, S> {
