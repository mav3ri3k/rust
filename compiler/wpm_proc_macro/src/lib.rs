...
#[rustc_diagnostic_item = "WpmTokenStream"]
#[stable(feature = "proc_macro_lib", since = "1.15.0")]
#[derive(Clone)]
pub struct TokenStream(pub Option<bridge::client::TokenStream>);
...
impl TokenStream {
    ...
    /// Some doc
    #[stable(feature = "proc_macro_lib2", since = "1.29.0")]
    pub fn ret_ts(&self) -> Option<bridge::client::TokenStream> {
        match self {
            TokenStream(Some(ts)) => Some(ts.clone()),
            TokenStream(None) => None,
        }
    }
