...
impl<'a> CrateLocator<'a> {
    ...
    fn find_library_crate(
        ...
        let dylib_prefix = &format!(
            "{}{}{}",
            self.target.dll_prefix, self.crate_name, extra_prefix
        );
        let wasm_prefix = &format!("pm{}{}", self.crate_name, extra_prefix);
        let staticlib_prefix = &format!(
            "{}{}{}",
        ...
        for search_path in self.filesearch.search_paths() {
            info!("searching {}", search_path.dir.display());
            for spf in search_path.files.iter() {
                info!("testing {}", spf.path.display());

                let f = &spf.file_name_str;
                let (hash, kind) = if f.starts_with(rlib_prefix) && f.ends_with(rlib_suffix) {
                    (
                        &f[rlib_prefix.len()..(f.len() - rlib_suffix.len())],
                        CrateFlavor::Rlib,
                    )
                } else if f.starts_with(rmeta_prefix) && f.ends_with(rmeta_suffix) {
                    (
                        &f[rmeta_prefix.len()..(f.len() - rmeta_suffix.len())],
                        CrateFlavor::Rmeta,
                    )
                } else if f.starts_with(dylib_prefix) && f.ends_with(dylib_suffix.as_ref()) {
                    (
                        &f[dylib_prefix.len()..(f.len() - dylib_suffix.len())],
                        CrateFlavor::Dylib,
                    )
                } else if f.starts_with(wasm_prefix) && f.ends_with(wasm_suffix) {
                    info!("Returning pm_wasm (hash, kind)");
                    (
                        &f[wasm_prefix.len()..(f.len() - wasm_suffix.len())],
                        CrateFlavor::Wasm,
                    )
                } else {
                ...
                match kind {
                    ...
                    CrateFlavor::Wasm => wasms.insert(path, search_path.kind),
                };
            }
        }
        ...
        for (_hash, (rlibs, rmetas, dylibs, wasms)) in candidates {
            if let Some((svh, lib)) = self.extract_lib(rlibs, rmetas, dylibs, wasms)? {
        ...
        for (lib, kind) in m {
            ...
            let (hash, metadata) = match get_metadata_section(
                ...
                Err(MetadataError::VersionMismatch {
                    ...
                    self.crate_rejections.via_version.push(CrateMismatch {
                        path: lib,
                        got: found_version,
                    });
                    continue;
                ...
                Err(MetadataError::LoadFailure(err)) => {
                    ...
                    // couldn't load it for some reason. Give a hard error instead of silently
                    // ignoring it, but only if we would have given an error anyway.
                    self.crate_rejections.via_invalid.push(CrateMismatch {
                        path: lib,
                        got: err,
                    });
                    continue;
    ...
    fn crate_matches(&mut self, metadata: &MetadataBlob, libpath: &Path) -> Option<Svh> {
        ...
        if let Some(expected_hash) = self.hash {
            if hash != expected_hash {
                info!(
                    "Rejecting via hash: expected {} got {}",
                    expected_hash, hash
                );
                self.crate_rejections.via_hash.push(CrateMismatch {
                    path: libpath.to_path_buf(),
                    got: hash.to_string(),
                });
                return None;
    ...
    fn find_commandline_library(&mut self) -> Result<Option<Library>, CrateError> {
        ...
        let mut wasms = FxHashMap::default();
        for loc in &self.exact_paths {
            /*
            if loc.canonicalized().extension().unwrap() == "wasm" {
                return Err(CrateError::ExternLocationWasmArtifact(
                    self.crate_name,
                    loc.original().clone(),
                ));
            }
            */
            ...
            if file.starts_with("lib") && (file.ends_with(".rlib") || file.ends_with(".rmeta"))
                ...
                || (file.starts_with("pm") && file.ends_with(".wasm"))
            {
                    ...
                    .ends_with(".rmeta")
                {
                    ...
                    .ends_with(".wasm")
                {
                    wasms.insert(loc_canon, PathKind::ExternFlag);
                    info!("Wasm file found");
                } else {
                    dylibs.insert(loc_canon, PathKind::ExternFlag);
                }
            } else {
                self.crate_rejections.via_filename.push(CrateMismatch {
                    path: loc.original().clone(),
                    got: String::new(),
                });
            }
        }

        // Extract the dylib/rlib/rmeta/wasm quadrapule.
        let library = self
            .extract_lib(rlibs, rmetas, dylibs, wasms)?
            .map(|(_, lib)| lib);
        if is_my_macro {
            info!("Library found: {:?}", library);
        };
        return Ok(library);
    }
...
fn get_metadata_section<'p>(
    ...
    let raw_bytes = match flavor {
        CrateFlavor::Rlib => loader
            .get_rlib_metadata(target, filename)
...
pub(crate) enum CrateError {
    ...
    #[allow(dead_code)]
    ExternLocationWasmArtifact(Symbol, PathBuf),
...
impl CrateError {
    pub(crate) fn report(self, sess: &Session, span: Span, missing_core: bool) {
        ...
        match self {
            ...
            CrateError::ExternLocationWasmArtifact(crate_name, loc) => {
                dcx.emit_err(errors::ExternLocationWasmArtifact {
                    span,
                    crate_name,
                    location: &loc,
                });
            ...
            CrateError::ExternLocationNotFile(crate_name, loc) => {
                dcx.emit_err(errors::ExternLocationNotFile {
                    span,
                    crate_name,
                    location: &loc,
                });
            ...
            CrateError::MultipleCandidates(crate_name, flavor, candidates) => {
                dcx.emit_err(errors::MultipleCandidates {
                    span,
                    crate_name,
                    flavor,
                    candidates,
            ...
            CrateError::SymbolConflictsCurrent(root_name) => {
                dcx.emit_err(errors::SymbolConflictsCurrent {
                    span,
                    crate_name: root_name,
                });
            }
            CrateError::StableCrateIdCollision(crate_name0, crate_name1) => {
                dcx.emit_err(errors::StableCrateIdCollision {
                    span,
                    crate_name0,
                    crate_name1,
                });
